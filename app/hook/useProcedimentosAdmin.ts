import { ProcedimentoService } from "app/api/services/procedimentoAdmin";
import { Procedimento } from "app/interfaces/profissionaisInterface";
...
export function useProcedimentosAdmin() {
  ...
  const updateProcedimento = useCallback(
    async (id: string, p: Omit<Procedimento, "id">) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        if (mounted.current && atualizado) {
          setProcedimentos((prev) =>
            prev.map((item) =>
              item.id === id ? (atualizado as Procedimento) : item
            )
          );
        }
  ...
  const getProcedimentosByProfissional = useCallback(
    (profissionalId: string) =>
      procedimentos.filter((p) => p.profissionalId === profissionalId),
    [procedimentos]
  );
